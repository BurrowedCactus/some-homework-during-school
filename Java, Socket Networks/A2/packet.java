...
public class packet {
	...
	public void sendPacketTo(String targetAddress, int targetPort, DatagramSocket localSocket) throws Exception {
		byte[] dataToBeSent = getUDPdata();
		localSocket.send(new DatagramPacket(dataToBeSent, dataToBeSent.length, InetAddress.getByName(targetAddress),
				targetPort));
	}

	public static packet getPacketFrom(DatagramSocket localSocket) throws Exception {
		byte[] dataToBeReceived = new byte[512];
		localSocket.receive(new DatagramPacket(dataToBeReceived, dataToBeReceived.length));
		return parseUDPdata(dataToBeReceived);
	}
}